...
class ChatViewController: QMChatViewController, QMChatServiceDelegate, UIActionSheetDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, QMChatAttachmentServiceDelegate, QMChatConnectionDelegate, QMChatCellDelegate, QMDeferredQueueManagerDelegate, QMPlaceHolderTextViewPasteDelegate {
    ...
    override func viewDidLoad() {
        ...
        let header = UIView(frame: CGRect(x: 0, y: 0, width: SCREEN_SIZE.width, height: IphoneWithNotch ? 98:78))
        view.addSubview(header)
        header.backgroundColor = .white
        self.view.backgroundColor = UIColor(hex: BACKGROUND_COLOR, alpha: 1)
        
        
        let btn = UIButton(frame: CGRect(x: 7, y: 40, width: 70, height: 30))
        header.addSubview(btn)
        btn.setImage(#imageLiteral(resourceName: "backArrow"), for: .normal)
        btn.setTitle("  Chats", for: .normal)
        btn.setTitleColor(.black, for: .normal)
        btn.contentHorizontalAlignment = .left
        btn.frame = CGRect(x: btn.frame.origin.x, y: header.frame.height - btn.frame.height - 10.0, width: btn.frame.width, height: btn.frame.height)
        btn.titleLabel?.font = UIFont(name: FONT_SF_DISPLAY_REGULAR, size: 18)
        btn.addTarget(self, action: #selector(self.btnBack(_:)), for: .touchUpInside)
        
        let profilePic = UIButton(frame: CGRect(x: SCREEN_SIZE.width - 18 - 44, y: header.frame.height - 44 - 10.0, width: 44, height: 44))
        profilePic.setImage(#imageLiteral(resourceName: "SamapleMan"), for: .normal)
        header.addSubview(profilePic)
        
        let lbl = UILabel(frame: CGRect(x: btn.frame.origin.x + btn.frame.width, y: header.frame.height - btn.frame.height - 10.0, width: SCREEN_SIZE.width - ((btn.frame.origin.x + btn.frame.width) * 2), height: 30))
        header.addSubview(lbl)
        lbl.text = "Donnie anderson"
        lbl.font = UIFont(name: FONT_SF_DISPLAY_SEMI_BOLD, size: 17)
    ...
    override func attributedString(forItem messageItem: QBChatMessage!) -> NSAttributedString? {
        ...
        attributes[NSAttributedStringKey.font] = UIFont(name: FONT_SF_DISPLAY_MEDIUM, size: 19)
    ...
    override func collectionView(_ collectionView: QMChatCollectionView!, configureCell cell: UICollectionViewCell!, for indexPath: IndexPath!) {
        ...
        if let attachmentCell = cell as? QMChatAttachmentCell {
            ...
            if attachmentCell is QMChatAttachmentIncomingCell {
                ...
                chatCell.containerView?.bgColor = fromIntToColor(red: 201, green: 206, blue: 230, alpha: 1)
