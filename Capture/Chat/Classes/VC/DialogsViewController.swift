...
class DialogTableViewCellModel: NSObject {
    ...
    init(dialog: QBChatDialog) {
        ...
        if dialog.type == .private {
            self.dialogIcon = UIImage(named: "SamapleMan")
        }
        else {
...
class DialogsViewController: UIViewController, QMChatServiceDelegate, QMChatConnectionDelegate, QMAuthServiceDelegate,UITableViewDelegate,UITableViewDataSource {
    ...
    func chatServiceChatDidAccidentallyDisconnect(_ chatService: QMChatService) {
        print("SA_STR_DISCONNECTED".localized)
    }
    
    func chatServiceChatDidConnect(_ chatService: QMChatService) {
        print("SA_STR_CONNECTED".localized)
        if !ServicesManager.instance().isProcessingLogOut! {
            self.getDialogs()
        }
    }
